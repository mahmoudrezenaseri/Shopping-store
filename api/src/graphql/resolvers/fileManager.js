...
var resolvers = {
    Query: {
        ...
        filterFiles: async (param, args, { req }) => {
            // check if user has logged in and is administrator
            if (!await common.checkIfAdmin(req, config.secretId)) {
                handleErrors(null, 403, "امکان استفاده از این بخش وجود ندارد");
                return;
            ...
            const media = await getFilesWithfilterHandler(args).
                catch(error => {
                    handleErrors(error, error.code, error.message)
                });
...
const getFilesWithfilterHandler = async (args) => {

    const page = args.page || 1;
    const limit = args.limit || 10;
    console.log(args.searchText)
    const media = await FileManager.paginate({ "name": { "$regex": args.searchText, "$options": "i" } }, { page, limit });
